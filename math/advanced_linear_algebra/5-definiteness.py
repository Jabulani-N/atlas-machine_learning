...
this module, 5-definiteness.py
calculates the inverse matrix of matrix matrix
utilizes task 4's code
numpy as np is permitted
"""
...
def definiteness(matrix):
    """calculates definiteness of ndarray matrix"""
    ...
    if np.sum(eigenvalues > 0) > 0:
        positives = np.sum(eigenvalues > 0)
    if np.sum(eigenvalues < 0) > 0:
        negatives = np.sum(eigenvalues < 0)

    if positives and negatives:
        return "Indefinite"
    elif positives == len(matrix):
        return "Positive definite"
    elif negatives == len(matrix):
...
def check_symmetric(a, rtol=1e-05, atol=1e-08):
    """
    checks if input ndaray is symmetric
    raises ValueError if not square
    """
    # print("Symmetry check says", np.allclose(a, a.T, rtol=rtol, atol=atol))
